...
var assigned = false;
ConsumerBuilder<Null, string> cb = new ConsumerBuilder<Null, string>(cConfig);
cb.SetErrorHandler((_, error) =>
{
    Log.Logger.Error($"An error ocurred consuming the event: {error.Reason}");
    if (error.IsFatal) Environment.Exit(-1);
});

cb.SetLogHandler((_, message) =>
{
    Log.Logger.Debug(message.Message);
});

using var consumer = cb
    .SetPartitionsAssignedHandler((c, ps) => { assigned = true; })
    .Build();

ProducerBuilder<Null, string> pbuilder = new ProducerBuilder<Null, string>(pConfig);
pbuilder.SetErrorHandler((_, error) =>
{
    Log.Logger.Error($"An error ocurred producing the event: {error.Reason}");
    if (error.IsFatal) Environment.Exit(-1);
});

pbuilder.SetLogHandler((_, message) =>
{
    Log.Logger.Debug(message.Message);
});
